...
class Table_transactions extends ActiveRecord implements TransactionsInterface
{
    ...
    public static function getTransactionsByBill(Table_payment_bills $info): array
    {
        if($info instanceof Table_payment_bills_double){
            return Table_transactions_double::findAll(['billId' => $info->id]);
        }
        return self::findAll(['billId' => $info->id]);
    }
}