...
class PowerHandler extends Model
{
    ...
    public function insert(): array
    {
        if ($this->validate()) {
            ...
            Telegram::sendDebug("Зарегистрировано потребление электроэнергии за " . mb_strtolower(TimeHandler::getFullFromShotMonth($newData->month)) . " года. Потребление: $newData->difference киловатт, на сумму " . CashHandler::toJsRubles($totalPay) . ' руб.');
            (new FirebaseHandler())->notifyMonthPowerUseRegistered($newData);
     ...
     * @throws Throwable
     */
    public static function cancelPowerFill($cottageNumber, $additional): array|bool
    {
        $transaction = new DbTransaction();
        if ($additional) {
     ...
     */
    private static function getMonthInfo(CottageInterface $cottage, ?string $period = null): Table_power_months
    {
        {
    ...
    public static function getUnfilled($period): Table_tariffs_power|bool
    {
     ...
     * @throws ErrorException
     */
    private function countCost(int $difference): float|bool|int
    {
     ...
     * @return float
     */
    #[Pure] private static function count($data, $tariff, $cottageNumber): float
    {
     ...
     * @return array
     */
    #[Pure] #[ArrayShape(['cost' => "float|int", 'over_cost' => "float|int", 'total' => "float|int"])] private static function countCostDetails(Table_power_months $existentValue, Table_tariffs_power $tariff): array
    {
    ...
    #[ArrayShape(['status' => "int", 'message' => "string"])] public static function discardCounterChange($cottageNumber, $targetMonth): array
    {
        $transaction = new DbTransaction();
        $cottageInfo = Cottage::getCottageByLiteral($cottageNumber);
        // проверю, проводилась ли замена счётчика
        $changeFact = Table_counter_changes::find()->where(['cottageNumber' => $cottageInfo->cottageNumber, 'changeMonth' => $targetMonth])->orderBy('change_time DESC')->one();
        if ($changeFact !== null) {
            $cottageInfo->currentPowerData = $changeFact->oldCounterNewData;
            // придётся проверять, не было ли несколько замен счётчика за месяц
            $changeFact->delete();
            // найду данные, введённые по участку за данный месяц
            $countedData = Table_power_months::find()->where(['cottageNumber' => $cottageInfo->cottageNumber, 'month' => $targetMonth])->one();
            if ($countedData !== null) {
            ...
            SingleHandler::removeDebtByName($cottageInfo, 'Оплата электроэнергии по старому счётчику за ' . TimeHandler::getFullFromShotMonth($targetMonth) . ' при замене на новый');
            $cottageInfo->save();
            $transaction->commitTransaction();
