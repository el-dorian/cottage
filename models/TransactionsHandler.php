...
class TransactionsHandler extends Model
{
    ...
    {
        if($cottage->isMain()){
            return Table_transactions::find()->where(['cottageNumber' => $cottage->getBaseCottageNumber()])->andWhere(['>=', 'payDate', $start])->andWhere(['<=', 'payDate', $end])->all();
        }
        return Table_transactions_double::find()->where(['cottageNumber' => $cottage->getBaseCottageNumber()])->andWhere(['>=', 'payDate', $start])->andWhere(['<=', 'payDate', $end])->all();
    }
    ...
    public static function getLastTransaction(string $cottageNumber, $item)
    {
        if(GrammarHandler::isMain($cottageNumber)){
            return Table_transactions::find()->where(['billId' => $item->id])->orderBy('payDate')->one();
        }
        return Table_transactions_double::find()->where(['billId' => $item->id])->orderBy('payDate')->one();
    }
