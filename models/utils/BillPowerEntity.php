...
class BillPowerEntity extends BillContentEntity
{
    ...
    public function getPayedOutside(): float
    {
        ...
        if(GrammarHandler::isMain($this->cottageNumber)){
            $items = Table_payed_power::find()->where([
                'cottageId' => $this->cottageNumber,
                'month' => $this->date
            ])->andWhere(['<>', 'billId',  $this->billId])->all();
        }
        else{
            $items = Table_additional_payed_power::find()->where([
    ...
    public function getPayedInside(): float
    {
        $sum = 0;
        if(GrammarHandler::isMain($this->cottageNumber)){
            $items = Table_payed_power::findAll(['billId' => $this->billId, 'cottageId' => $this->cottageNumber, 'month' => $this->date]);
        }
        else{
            $items = Table_payed_power::findAll(['billId' => $this->billId, 'cottageId' => (int) $this->cottageNumber, 'month' => $this->date]);
        }
        if($items !== null){
            foreach ($items as $item) {
                $sum += $item->summ;
