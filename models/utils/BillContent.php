...
class BillContent
{
    ...
    public function decompile($text): void
    {
        ...
        // пени
        $fines = Table_bill_fines::findAll(['bill_id' => $this->info->id]);
        if (!empty($fines)) {
            foreach ($fines as $fine) {
                $newItem = $this->handleFineEntity($fine);
                $newItem->totalAccrued = $fine->start_summ;
                $this->fineEntities[] = $newItem;
            }
        }
    }
    ...
    private function handleFineEntity(Table_bill_fines $paymentItem): BillFineEntity
    {
        $newEntity = new BillFineEntity();
        $fineInfo = Table_penalties::findOne($paymentItem->fines_id);
        if ($fineInfo !== null) {
            $newEntity->fine_id = $paymentItem->fines_id;
            $newEntity->billId = $paymentItem->bill_id;
            $newEntity->date = $fineInfo->period;
            $newEntity->payed = $fineInfo->is_full_payed;
            $newEntity->payedBefore = $newEntity->getPayedInside();
            $newEntity->sum = $paymentItem->start_summ;
            $newEntity->leftToPay = $paymentItem->start_summ - $fineInfo->payed_summ;
            return $newEntity;
        }
        return $newEntity;
    }
    ...
    public function getTextContent(): string
    {
        ...
        if (!empty($this->fineEntities)) {
            foreach ($this->fineEntities as $entity) {
                $answer .= $entity->getTextContent();
            }
        }
        return $answer;
    }
    ...
    public function getRequiredSum()
    {
        ...
        if (!empty($this->fineEntities)) {
            foreach ($this->fineEntities as $entity) {
                $payedOutside += $entity->getPayedOutside();
            }
        }
        $result -= $payedOutside;
        return $result;
