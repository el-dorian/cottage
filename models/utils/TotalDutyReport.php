        ...
        if (!empty($cottages) && count($cottages) > 0) {
            foreach ($cottages as $cottage) {
                ...
                if($cottage->isMain()){
                    ...
                    $powerBills = Table_additional_power_months::find()->where(['cottageNumber' => $cottage->cottageNumber])->andWhere(['<', 'searchTimestamp', $timestamp])->all();
                }
                if (!empty($powerBills)) {
                    ...
                    foreach ($powerBills as $powerBill) {
                        ...
                        if ($powerBill->totalPay > 0) {
                            ...
                            if($cottage->isMain()){
                                ...
                                $payments = Table_additional_payed_power::find()->where(['cottageId' => $cottage->cottageNumber, 'month' => $powerBill->month])->andWhere(['<', 'paymentDate', $timestamp])->all();
                            }
                            if (!empty($payments)) {
                ...
                if ($totalPowerDuty > 0) {
                    $powerItem = '<участок><номер>' . $cottage->cottageNumber . '</номер>';
                    $powerItem .= '<электричество_общая_задолженность>' . CashHandler::toRubles($totalPowerDuty) . '</электричество_общая_задолженность>' . $powerDutyDetails;
                    $powerItem .= '</участок>';
                    $powerDetailsXml .= $powerItem;
                }
                // ЧЛЕНСКИЕ ВЗНОСЫ =====================================================================================
                ...
                if($cottage->isMain()){
                    ...
                    $overpays = Table_additional_payed_membership::find()->where(['cottageId' => $cottage->cottageNumber])->andWhere(['<', 'paymentDate', $timestamp])->all();
                }
