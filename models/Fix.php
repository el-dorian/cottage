...
//    }
    ...
    public static function fixAccruals()
    {
        $pastAccruals = Accruals_membership::find()->where(['>=', 'quarter', TimeHandler::getCurrentQuarter()])->all();
        if(!empty($pastAccruals)){
            foreach ($pastAccruals as $pastAccrual) {
                $cottage = Cottage::getCottageByLiteral($pastAccrual->cottage_number);
                $pastAccrual->counted_square = $cottage->cottageSquare;
                $pastAccrual->save();
            }
        }
    }
