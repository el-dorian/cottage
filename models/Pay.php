...
class Pay extends Model
{
    ...
    public static function reopenBill($billId, $double): array
    {
        ...
        if ($billInfo !== null) {
            ...
            if ($billInfo->depositUsed > 0 && CashHandler::toRubles($billInfo->payedSumm) == 0) {
     ...
     * @throws Exception
     */
    #[ArrayShape(['status' => "int", 'message' => "string"])] public function confirm(): ?array
    {
        ...
        try {
            ...
            if ($this->sendConfirmation) {
                ...
                (new FirebaseHandler())->sendNewTransactionConfirmedNotification($billTransaction);
            }
            Telegram::sendDebug('Зарегистрирован платёж на сумму ' . CashHandler::toSmoothRubles($billTransaction->transactionSumm) . " участка $billTransaction->cottageNumber по счёту $billTransaction->billId");
