...
class Filling extends Model
{
    ...
    public static function getPaymentDetails($billInfo): string
    {
        ...
        if ($summChanged) {
            $content .= self::getRow('<b>Итого к оплате</b>',
                CashHandler::toSmoothRubles(
                    CashHandler::toRubles(
                        CashHandler::toRubles($info['billInfo']->totalSumm) -
                        CashHandler::toRubles($info['billInfo']->payedSumm) -
                        CashHandler::toRubles($info['billInfo']->depositUsed) +
                        CashHandler::toRubles($info['billInfo']->toDeposit) -
                        CashHandler::toRubles($info['billInfo']->discount))
                ), Colors::COLOR_DATA);
        }
