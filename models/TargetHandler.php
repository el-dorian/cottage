...
class TargetHandler extends Model
{
    ...
    {
        if ($cottage->isMain()) {
            return Table_payed_target::find()->where(['year' => $year, 'cottageId' => $cottage->getBaseCottageNumber()])->andWhere(['<=', 'paymentDate', $periodEnd])->orderBy('paymentDate')->all();
        }
    ...
    public function createTariff(): bool
    {
        if ($this->validate()) {
            ...
            foreach ($cottages as $cottage) {
                // добавлю начисление
                $accrual = $this->createTargetAccrual($cottage);
                (new FirebaseHandler())->notifyNewTargetAccrual($cottage, $accrual, $newTariff);
            }
            $additionalCottages = AdditionalCottage::getRegistred();
            foreach ($additionalCottages as $cottage) {
                if ($cottage->isTarget) {
            ...
            Telegram::sendDebug("Добавлен целевой платёж за $newTariff->year, стоимость с участка: $newTariff->fixed_part р., стоимость с сотки: $newTariff->float_part, цель: $newTariff->description");
            return true;
        }
...
//    }
    /**
     * @param \app\models\interfaces\CottageInterface $cottage
     * @return \app\models\database\Accruals_target
     */
    public function createTargetAccrual(CottageInterface $cottage): Accruals_target
    {
