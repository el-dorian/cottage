        ...
        if ($bankTransaction !== null) {
            ...
            if ($cottage !== null) {
                ...
                $payTransaction = new Table_transactions([
                    ...
                    'bankDate' => TimeHandler::getCustomTimestamp($bankTransaction->pay_date),
                    'payDate' => TimeHandler::getCustomTimestamp($bankTransaction->real_pay_date),
                    'transactionDate' => TimeHandler::getCustomTimestamp($bankTransaction->pay_date),
                    ...
                    'transactionType' => 'no-cash',
                    'partial' => 0,
                    'transactionReason' => 'Зачисление на депозит банковской транзакции',
                    'transactionWay' => 'in',
                    'usedDeposit' => 0
                ]);
                $payTransaction->save();
                ...
                $depositIo = new Table_deposit_io([
                    ...
                    'actionDate' => TimeHandler::getCustomTimestamp($bankTransaction->pay_date)
                ]);
                $depositIo->save();
...
// создам транзакцию
            ...
            if (!empty($billContentInfo->powerEntities)) {
                foreach ($billContentInfo->powerEntities as $powerEntity) {
                    ...
                    $leftToPay = CashHandler::toRubles($powerEntity->getLeftToPay());
                    if ($leftToPay > 0) {
            ...
            $cottageInfo->save();
            $transaction->commitTransaction();
            Telegram::sendDebug('Зарегистрирован платёж на сумму ' . CashHandler::toSmoothRubles($t->transactionSumm) . " участка $t->cottageNumber по счёту $t->billId");
            if ($this->sendConfirmation === 'true') {
                (new FirebaseHandler())->sendNewTransactionConfirmedNotification($t);
