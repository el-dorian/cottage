...
class Utils extends Model
{
    ...
    public static function sendInfoToApi($cottageNumber): array
    {
        ...
        $cottage = \app\models\database\Cottage::findOne($cottageNumber);
        if ($cottage !== null) {
            $cottageItem = [];
            ...
            // put data in file
            if (!is_dir(dirname(Yii::getAlias('@webroot') . './/') . '/output') && !mkdir($concurrentDirectory = dirname(Yii::getAlias('@webroot') . './/') . '/output') && !is_dir($concurrentDirectory)) {
            ...
            // send data to server
            $ch = curl_init("https://oplepiha.me/input");
...
# Print response.
    ...
    /**
     * Выполню резервное копирование базы данных
     *
     */
