...
class TransactionsDetailsWidget extends Widget
{
    ...
    public function run()
    {
        ...
        foreach ($this->info as $item) {
            echo "<tr><td>$item->id</td><td>" . TimeHandler::getDatetimeFromTimestamp($item->transactionDate) . "</td><td>" . CashHandler::toSmoothRubles($item->transactionSumm) . "</td><td>" . CashHandler::toSmoothRubles($item->usedDeposit) . "</td><td>" . CashHandler::toSmoothRubles($item->gainedDeposit) . "</td></tr>";
            echo "<tr><td colspan='5'></td><table class='table table-condensed table-striped'><caption>Оплачено в транзакции</caption>";
