...
class ReportController extends Controller
{
    ...
    public function actionRemindTarget(): string
    {
        // получу участки, которые не оплатили текущий квартал
        $debtors = Accruals_target::getDebtors();
        return $this->render('target_debtors_list', ['debtors' => $debtors]);
    }
    ...
    public function actionSendMembershipRemind($cottageNumber): array
    {
        ...
        MailingSchedule::addSingleMailing($cottageInfo, 'Срок оплаты взносов', 'Здравствуйте.<br/>' . 'Сегодня последний день установленного срока для оплаты членского взноса за ' . TimeHandler::getFullFromShortQuarter(TimeHandler::getCurrentQuarter()) . '. С завтрашнего дня за несвоевременную оплату в соответствии с Уставом СНТ начисляются пени. Пожалуйста, оплатите взносы в кратчайшие сроки.');
        return ['status' => 1];
    }
    public function actionSendTargetRemind($cottageNumber): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $cottageInfo = Cottage::getCottageByLiteral($cottageNumber);
        MailingSchedule::addSingleMailing($cottageInfo, 'Срок оплаты взносов', '
        Сегодня последний день установленного срока для оплаты целевого взноса за этот год. Пожалуйста, не забывайте производить платежи своевременно. За несвоевременную оплату в соответствии с Уставом СНТ  начисляются пени.');
        return ['status' => 1];
    }
